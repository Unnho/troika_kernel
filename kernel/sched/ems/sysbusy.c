...
/* 4.14 kernel compat: cpumask_last/cpumask_any_distribute not available */
...
/* CPU mask helpers for Exynos 9610 topology */
const struct cpumask *cpu_fastest_mask(void)
{
	return cpu_coregroup_mask(MAX_CAPACITY_CPU);
}

const struct cpumask *cpu_slowest_mask(void)
{
	return cpu_coregroup_mask(MIN_CAPACITY_CPU);
}

bool et_cpu_slowest(int cpu)
{
...
{
	return get_cpu_max_capacity(MAX_CAPACITY_CPU);
}
...
int is_somac_ready(struct task_struct *p)
{
	struct cpumask mask;
	...
	cpumask_and(&mask, &mask, &p->cpus_allowed);
	if (cpumask_empty(&mask))
		return 0;

	cpumask_set_cpu(cpumask_last(&mask), &mask);

	return 1;
}

static void update_somac_ready(struct task_struct *p)
{
	unsigned long flags;
	int cgroup_idx;
	...
	if (cgroup_idx != CGROUP_TOPAPP)
		return;

	spin_lock_irqsave(&somac_ready_lock, flags);
	if (!somac_ready.p || p->pid < somac_ready.p->pid)
		somac_ready.p = p;
...
get_remained_task_util(int cpu, struct task_struct *p)
{
	struct task_struct *excluded = p;
...

	{
		...
		list_for_each_entry(tp, &rq->cfs_tasks, se.group_node) {
			if (tp == excluded)
				continue;

			util = ml_task_util(tp);
			util_sum += util;
...
static int sysbusy_find_fastest_cpu(struct task_struct *p, int prev_cpu)
{
	struct cpumask mask, cpus_allowed;
	int cpu;

	cpumask_copy(&cpus_allowed, &p->cpus_allowed);
	cpumask_and(&mask, cpu_fastest_mask(), &cpus_allowed);
	if (cpumask_empty(&mask))
		cpu = cpumask_last(&cpus_allowed);
	else
	...
	return check_busy(cpu_util_wake(cpu, p),
				capacity_orig_of(cpu)) ? -1 : cpu;
}

static int sysbusy_find_min_util_cpu(struct task_struct *p, int prev_cpu)
{
	int cpu, min_cpu = -1;
	...
	for_each_cpu(cpu, &p->cpus_allowed) {
		...
		cpu_util = cpu_util_wake(cpu, p);

		if (cpu == task_cpu(p) && !cpu_util)
			cpu_util = get_remained_task_util(cpu, p);
...
int sysbusy_schedule(struct task_struct *p, int prev_cpu)
{
	int target_cpu = -1;
	...
	task_util_val = task_util_est(p);

	switch (sysbusy.state) {
	case SYSBUSY_STATE1:
		if (is_heavy_task_util(task_util_val))
			target_cpu = sysbusy_find_fastest_cpu(p, prev_cpu);
		break;
	case SYSBUSY_STATE2:
		target_cpu = sysbusy_find_min_util_cpu(p, prev_cpu);
		break;
	case SYSBUSY_STATE3:
		if (is_heavy_task_util(task_util_val)) {
