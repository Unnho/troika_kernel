...
/* Simplified cgroup_parse_float for 4.14 kernel */
static int cgroup_parse_float(const char *buf, unsigned int frac_shift,
			      s64 *endp)
{
	s64 val;
	int ret;

	ret = kstrtos64(buf, 10, &val);
	if (ret)
		return ret;

	{
		...
		if (dot) {
			...
			val = val * base + frac;
			val = val * (1 << frac_shift) / base;
			if (endp)
				*endp = (s64)(p - buf);
		} else {
			val = val * (1 << frac_shift);
			if (endp)
				*endp = strlen(buf);
		}
	}
...
static ssize_t cpu_uclamp_min_read(struct kernfs_open_file *of,
				   char *page, size_t nbytes, loff_t off)
{
	struct task_group *tg = css_tg(of_css(of));
	unsigned long req_value;

	req_value = tg->uclamp_req[UCLAMP_MIN].value;

	return scnprintf(page, nbytes, "%u\n", req_value);
}

static ssize_t cpu_uclamp_max_read(struct kernfs_open_file *of,
				   char *page, size_t nbytes, loff_t off)
{
	struct task_group *tg = css_tg(of_css(of));
	unsigned long req_value;

	req_value = tg->uclamp_req[UCLAMP_MAX].value;

	return scnprintf(page, nbytes, "%u\n", req_value);
}
#endif /* CONFIG_CGROUP_V2 */
...
recheck:
	...
	if (attr->sched_flags &
		~(SCHED_FLAG_RESET_ON_FORK | SCHED_FLAG_RECLAIM | SCHED_FLAG_UTIL_CLAMP))
		return -EINVAL;
